...
export default function ExpensesPage() {
    ...
    const expenseTypes = ['PERSONAL', 'INVESTMENT', 'MEDICINE', 'FOOD', 'ENTERTAINMENT', 'TRAVEL', 'OTHER'];
    ...
    const fetchExpenses = async () => {
        try {
            ...
            if (sortBy === 'date') params.sortBy = 'expenseDate';
            else if (sortBy === 'amount') params.sortBy = 'expenseAmount';
            else if (sortBy) params.sortBy = sortBy;

            const response = await api.get('/user/expense/get', { params });
    ...
    const handleDelete = async (id) => {
        ...
        try {
            await api.delete(`/user/expense/delete/${id}`);
            fetchExpenses();
        } catch (error) {
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            await api.post('/user/expense/add', formData);
        ...
        <div className="max-w-6xl mx-auto">
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-4">
                ...
                <button
                    ...
                    className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-4 py-2.5 rounded-lg transition-colors shadow-sm"
                >
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
                {loading ? (
                    ...
                    <div className="p-12 text-center">
                        ...
                        <p className="text-sm text-gray-500">Add a new expense to get started</p>
                    </div>
                ) : (
                        ...
                        <table className="w-full text-left">
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {expenses.map((expense) => (
                                    <tr key={expense.expenseId} className="hover:bg-gray-50/50 transition-colors">
                                        <td className="px-6 py-4 text-sm text-gray-900 font-medium">{expense.expenseDesc}</td>
